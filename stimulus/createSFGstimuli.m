...
% presence / absence of figure is controlledwith figure coherence 
% (figureCoh field) - if set to 0, there is no figure in the stimulus.  
...
% sampleFreq, chordOnset, chordDur, figureMinOnset, figureOnsets, 
...
% if there is no value in stimopt.figureOnsets, set it to nan
if isempty(stimopt.figureOnsets)
    stimopt.figureOnsets = nan;
end
% check the range of figureOnsets
if any(stimopt.figureOnsets < ceil(stimopt.figureMinOnset/stimopt.chordDur))
    error('At least one figure onset value is below the minimum onset value!');
end
...
for stimNo = 1:NStimuli
    ...
    if isnan(stimopt.figureOnsets)
        ...
        figureStartInterval = stimopt.figureOnsets;
        figureEndInterval   = figureStartInterval + stimopt.figureDur - 1;
    end

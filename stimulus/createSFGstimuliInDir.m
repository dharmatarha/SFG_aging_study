function wavDir = createSFGstimuliInDir(NStimuli, stimopt, wavDir, loudnessEq)
%% Creates a sequence of Nstimuli stochastic figure-ground (SFG) stimulus.
% 
% USAGE: wavDir = createSFGstimuli(NStimuli, stimopt, wavDir, loudnessEq=false)
%
...
%
% Mandatory inputs:
% NStimuli      - Numeric value, number of stimuli to be generated
% stimopt       - Struct containing stimulus parameters (both for 
%               background and figure). For details see SFGparams
% wavDir        - Char array, name of the folder with the generated stimuli
%
% Optional input:
...
if ~ismembertol(nargin, 3:4)
    error('Function createSFGstimuli requires input args "NStimuli", "stimopt" and "wavDir" while input arg "loudnessEq" is optional!');
end
if nargin == 3
    loudnessEq = false;
end
...
disp([char(10), 'Called createSFGstimuli function with input args:',...
    ...
    char(10), 'Loudness correction is set to: ', num2str(loudnessEq),...
    char(10), 'stimopt: ']);
disp(stimopt);
...
% save out detailed info about random background + figure chords + loudness
% correction flag
chordInfoFile = ['./', wavDir, '/', wavDir, '_chordInfo.mat'];
save(chordInfoFile, 'allBackgrFreqs', 'stimulusChordNumber',... 
    'NStimuli', 'allFigFreqs', 'loudnessEq');

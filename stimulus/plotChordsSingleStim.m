function fig = plotChordsSingleStim(audioData, stimopt, allFigFreqs, allBackgrFreqs)
%% Plot background + figure chords for SFG stimulus
%
% USAGE: fig = plotChordsSingleStim(audioData, stimopt, allFigFreqs, allBackgrFreqs)
%
...
% spectrogram. Assumes that raw audio and the freqnecy component matrices 
% were generated with createSingleSFGstimuli.m
%
% Inputs:
% audioData         - Numeric matrix, audio data. Sized 1/2 X samples 
%                   (mono or stereo)
% stimopt           - Struct holding stimulus parameters in, same as used 
%                   with createSingleSFGstim.m
...
%                   numeric matrix. Output of createSingleSFGstim.m
%
% Output is a figure with two subplots.
%
...
snr = stimopt.figureCoh/stimopt.toneComp;  % signal-to-noise ratio

% frequency range used
freqLimits = [stimopt.toneFreqMin, stimopt.toneFreqMax];

% sanity checks
if ~isequal(size(audioData, 2), stimopt.totalDur*fs)
...
disp([char(10), 'Properties of SFG stimulus according to parameters in stimopt:', char(10),...
    'Figure coherence level: ', num2str(stimopt.figureCoh), char(10),...
    'Figure step size: ', num2str(stimopt.figureStepS), char(10),...
    'Figure duration: ', num2str(stimopt.figureDur), ' chords', char(10),...
    'Figure starts at chord: ', num2str(figStart), char(10),...
    'Figure ends at chord: ', num2str(figEnd), char(10),...
    'Chord duration: ', num2str(stimopt.chordDur*1000), ' ms', char(10),...
...
% subplot details
title(['Background and figure chords used for SFG stimulus', char(10)]);
xlabel('Chord number');
ylabel('Frequency');
hold off;
set(gca, 'FontSize', 14);
...
% subplot details
title(['Spectrogram of SFG stimulus', char(10)]);
hold off;

% Set overall figure features
set(gcf,'color','w');
figureTitle = ['Coh_', num2str(stimopt.figureCoh), '__Dur_', num2str(stimopt.figureDur),...
    '__toneComp_', num2str(stimopt.toneComp)];

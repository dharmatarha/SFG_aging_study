function [stimArray, sortIndices, startTrialNo,... 
    ...
    stimTypes, stimTypeIdx] = expParamsHandler(subNum, stimArrayFile, blockNo)
%% Function for SFG exp. handling parameters/settings, stimuli and conflicts
% 
...
% startTrialNo  - Trial number the expeirment starts from
...
returnFlag = 0;
% pre-set certain return variables to defaults in case of early return
startTrialNo = 1; startBlockNo = 1;
logVar = {}; blockIdx = []; trialIdx = [];
stimArray = []; sortIndices = [];
    ...
    if exist(subParamsF, 'file') 
        ...
        if isequal(oldParams.stimArrayFile, stimArrayFile) && isequal(oldParams.blockNo, blockNo) && ...
            ...
            if exist(subLogF, 'file')
                oldLog = load(subLogF);
                % check if stored log is expected format - sane header? size?
                if isequal(oldLog.logVar(1, :), logHeader) && size(oldLog.logVar, 1)>1
                    logFileFlag = 1;
...
elseif oldParamsFileFlag && oldParamsMatchFlag && logFileFlag
    ...
    inputRes = input([char(10), 'What should we do? (1 = Simply go on with the ',...
        'experiment; 2 = Exit, so I can take a better look at the situation']);
    if isequal(inputRes, 1)
...
% Params/settings:
% Existing parameters would have to match the new ones we would
% generate - so we can just generate params and load stimuli as we would do
% it in the absence of any subject folder/params file

% user message
disp([char(10), 'We load the stimuli, perform checks on it and sort them to blocks (if necessary)']);
...
% transform stimTypes varialbe into human readable form - cell array with
% headers
stimTypesHdr = {'figDuration', 'figCoherence', 'figPresence', 'stimulusTypeIndex'};
stimTypes = [stimTypesHdr; num2cell([stimTypes, [1:12]'])];

% save basic parameters
save(subParamsF, 'trialIdx', 'blockIdx', 'stimTypes', 'stimTypeIdx',... 
    'randomseed', 'stimArrayFile', 'timestamp', 'blockNo', 'subNum');
% user message
disp([char(10), 'Loaded stimuli and created a parameters/settings file']);


%% Init logging/result variable if there was none
...
% If the last block was finished (and the block was not the last one) we 
% start with the next block. Otherwise we restart the last block.

% only if there was a valid log file
if logFileFlag
    % get the number of the last trial and the corresponding block based on
    % recorded response times and accuracy (double check for sanity)
    ...
    % get trial number
    loggedRTtrials = size(logRT, 1);
    % get recorded trial and block numbers for last data point
    loggedTrialNo = cell2mat(logVar(loggedRTtrials+1, strcmp(logHeader, 'trialNo')));  % get the number of the last one  
    loggedBlockNo = cell2mat(logVar(loggedRTtrials+1, strcmp(logHeader, 'blockNo')));  % get the number of the last one  
    % get the trial indices for the block
    ...
    elseif isequal(max(trialList), loggedTrialNo) && ~isequal(loggedBlockNo, blockNo)
        ...
        disp([char(10), 'Based on the log file, the subject just finished block ', num2str(loggedBlockNo),... 
            ' when the script finished.', char(10), 'We restart from the next block. Setting ',...
            'trial number to the first one in block ', num2str(loggedBLockNo+1)]);
        % set starting point to the first trial of the next block
        ...
        disp([char(10), 'Based on the log file, the subject was performing block ', num2str(loggedBlockNo),... 
            ' but did not finish it.', char(10), 'We restart from the beginning of that block. Setting ',...
            'trial number to the first one in block ', num2str(loggedBlockNo)]);
        % set starting point to the start of the block
        startTrialNo = min(trialList);
        startBlockNo = loggedBlockNo;
        % user message
